...
openAction = {'name'       : "openAction",
                ...
                'invariant'  : True,
                'controlVar' : [], #[open],
                'controlPred': And(open,Implies(t==2, Not(open))),     #this is temp hack. Otherwise chooses U = ~open
                'precNode'   : singleton,
                'postNode'   : singleton
                }
...
safetyProps = [ 0 <= tank, 
               #  0 <= t, t <= 2,
                Implies(And(t==0, open), tX==1),
                # Implies(And(t==0, Not(open)), tX==0),
                # Implies(And(t==1, Not(open)), tX==2),
                # Implies(And(t==1, open), tX==0),      #if its not closed, reset
               #  Implies(t==2, tX==0)
